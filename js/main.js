...
$(document).ready(function() {
    const drop = $('#sort-btn');
    const dropItems = $('.sort-btn-drop__items');
  
    drop.on('click', function() {
      dropItems.toggleClass('active');
      if (dropItems.hasClass('active')) {
        const itemCount = dropItems.find('.drop-item').length;
        dropItems.css('height', itemCount * 34 + 'px');
      } else {
        dropItems.css('height', '0');
      }
    });
  });
jQuery(function ($) {
...
// const mediaQueryHeader = window.matchMedia('(max-width: 700px)')
// let navbarBrand = document.querySelector('.navbar-brand')

// function handleTabletChange(e){
//     if(e.matches){
//         navbarBrand.innerHTML = 'WIUT'
//     }else{
//         navbarBrand.innerHTML = `Westminster International University<i class="version-number">in Tashkent</i>`
//     }
// }

// mediaQueryHeader.addListener(handleTabletChange)
// handleTabletChange(mediaQueryHeader)